...
c     ###############################################################
c     ##                                                           ##
c     ##  subroutine echarge1dh  --  charge derivs via debye huckel  ##
c     ##                                                           ##
c     ###############################################################
c
c
c     "echarge1dh" calculates the charge-charge interaction energy
c     and first derivatives with respect to Cartesian coordinates
c     using debye huckel
c
c
      subroutine echarge1dh
      use sizes
      use atoms
      use bound
      use cell
      ...
      real*8 rc5,rc6,rc7
      real*8 vxx,vyy,vzz
c      real*8 vyx,vzx,vzy,kdebyescreen
      real*8 vyx,vzx,vzy
...
c
            if (proceed) then
               ...
               if (rc2 .le. off2) then
                  ...
                  e = fik / rb
                  e = e*exp(-rb/kdebyescreen)
                  de = -e*(1/rb+1/kdebyescreen)
                  dc = 0.0d0
c
c     use shifted energy switching if near the cutoff distance
c
c                  shift = fik / (0.5d0*(off+cut))
c                  e = e - shift
c                  if (rc2 .gt. cut2) then
c                     rc = sqrt(rc2)
c                     rc3 = rc2 * rc
c                     rc4 = rc2 * rc2
c                     rc5 = rc2 * rc3
c                     rc6 = rc3 * rc3
c                     rc7 = rc3 * rc4
c                     taper = c5*rc5 + c4*rc4 + c3*rc3
c     &                          + c2*rc2 + c1*rc + c0
c                     dtaper = 5.0d0*c5*rc4 + 4.0d0*c4*rc3
c     &                           + 3.0d0*c3*rc2 + 2.0d0*c2*rc + c1
c                     trans = fik * (f7*rc7 + f6*rc6 + f5*rc5 + f4*rc4
c     &                               + f3*rc3 + f2*rc2 + f1*rc + f0)
c                     dtrans = fik * (7.0d0*f7*rc6 + 6.0d0*f6*rc5
c     &                               + 5.0d0*f5*rc4 + 4.0d0*f4*rc3
c     &                             + 3.0d0*f3*rc2 + 2.0d0*f2*rc + f1)
c                     dc = (e*dtaper + dtrans) / rc
c                     de = de * taper
c                     e = e*taper + trans
c                  end if
c
