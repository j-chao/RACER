...
c     #############################################################
c     ##                                                         ##
c     ##  subroutine ecghbond3  --  CG H-bond potential energy    ##
c     ##                                                         ##
c     #############################################################
c
c
...
c              print *, "line 156", type(ib), type(jb)
              xiab = xia - xib
              yiab = yia - yib
              ziab = zia - zib
              xicb = xic - xib
...
c               if (debug) then
c                write (*,*) "Energy CGHBOND", e, ib,jb, rhbond,
...
c
               if (debug) then
              write (*,*) "Energy CGHBOND", e, ib,jb, rhbond,
     &             radian *anglei, radian *anglej, radian *anglek

c     write out energy , atom number1, atom number2, Hbond distance, and
c     angle1 in rads, angle 2 in rads, and the final angle in rads
               end if
           end if 
  30       continue
          end do
